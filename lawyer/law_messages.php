...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['receiver_id'], $_POST['message_text'])) {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE assigned_lawyer = :lawyer_id AND (reported_by = :other_id OR assigned_to = :other_id)
    ");
    $stmt->execute(['lawyer_id' => $user_id, 'other_id' => $receiver_id]);
    if ($stmt->fetch()) {
...
$pdo->prepare("UPDATE messages SET is_read = TRUE WHERE receiver_id = ?")->execute([$user_id]);
...
<div class="dashboard-container">
    ...
    <?php if (count($conversations) > 0): ?>
        ...
        <a href="law_messages_convo.php?user=<?= urlencode($msg['conversation_with']) ?>" 
