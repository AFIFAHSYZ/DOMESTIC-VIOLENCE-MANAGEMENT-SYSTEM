...
// Check if police has access to this user based on shared case
$accessCheck = $pdo->prepare("
    SELECT 1 FROM dv_case
    WHERE assigned_to = :police_id AND (reported_by = :reported OR assigned_lawyer = :lawyer)
");
$accessCheck->execute([
    'police_id' => $user_id,
    'reported' => $other_id,
...
$stmt = $pdo->prepare("SELECT full_name FROM SYS_USER WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message_text'])) {
    ...
    header("Location: police_mess_convo.php?user=$other_id");
    exit();
}
...
<div class="dashboard-container">
    ...
    <main class="main-content">
        ...
        <a href="police_messages.php" class="btn">← Back to Messages</a>
