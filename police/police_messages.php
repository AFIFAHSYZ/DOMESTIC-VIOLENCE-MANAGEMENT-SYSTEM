...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['receiver_id'], $_POST['message_text'])) {
    ...
    // Validate access to receiver
    $stmt = $pdo->prepare("
        SELECT 1 FROM dv_case 
        WHERE assigned_to = :police_id AND (reported_by = :other_id OR assigned_lawyer = :other_id)
    ");
    $stmt->execute(['police_id' => $user_id, 'other_id' => $receiver_id]);
    if ($stmt->fetch()) {
...
$receivers = $pdo->prepare("
    ...
    INNER JOIN dv_case c 
        ON u.user_id = c.reported_by 
        OR u.user_id = c.assigned_lawyer
    WHERE c.assigned_to = :uid
      AND u.user_id != :uid
");
$receivers->execute(['uid' => $user_id]);
$receivers = $receivers->fetchAll();

// Fetch conversations only with allowed users
...
<?php if (count($conversations) > 0): ?>
    <div class="messages-list">
        <?php foreach ($conversations as $msg): ?>
            <a href="police_mess_convo.php?user=<?= urlencode($msg['conversation_with']) ?>" 
               class="conversation-item <?= $msg['is_read'] ? 'read' : 'unread' ?>">
               
            <img class="avatar-circle" 
                src="../get_profilepic.php?id=<?= urlencode($msg['conversation_with']) ?>&t=<?= time() ?>" 
                ...
                <?php if (!empty($msg['unread_count']) && $msg['unread_count'] > 0): ?>
                    <div class="badge"><?= $msg['unread_count'] ?></div>
                <?php endif; ?>
